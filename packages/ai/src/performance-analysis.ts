 ...
 */
export async function analyzeSellerPerformance(
  tenantId: string,
  sellerId: string,
  apiKey: string
): Promise<{
...
${completedSales.slice(0, 10).map(s => 
  `- ${(s as any).vehicle?.make || 'N/A'} ${(s as any).vehicle?.model || 'N/A'} vendido por $${s.salePrice || s.total || 0}`
).join('\n')}
 ...
 */
export async function suggestContinuousImprovements(
  tenantId: string,
  apiKey: string
): Promise<{
  improvements: Array<{
 ...
 */
export async function performAutoBenchmarking(
  tenantId: string,
  apiKey: string
): Promise<{
  benchmarks: Array<{
