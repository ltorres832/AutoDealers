...
export async function personalizeMessage(
  tenantId: string,
  leadId: string,
  messageType: 'greeting' | 'followup' | 'promotion' | 'closing',
  apiKey: string,
  ...
  try {
    ...
    const interestedVehicles = vehicles.filter(v => 
      leadData?.interestedVehicles?.includes(v.id)
    );
...
- Interacciones previas: ${interactions.slice(-3).map((i: any) => i.content).join('\n')}
 ...
 */
export async function recommendVehiclesByHistory(
  tenantId: string,
  leadId: string,
  apiKey: string
): Promise<{
  ...
  try {
    ...
    const vehicleList = vehicles.slice(0, 20).map(v => 
      `ID: ${v.id} - ${v.year} ${v.make} ${v.model}, Precio: $${v.price}, ${v.mileage ? `${v.mileage} km` : 'Nuevo'}`
    ).join('\n');

    const prompt = `Recomienda vehículos para este cliente basado en su historial:

Cliente: ${leadData?.contact?.name}
Interacciones: ${interactions.map((i: any) => i.content).join('\n')}
...
Responde en formato JSON con recommendedVehicles (array):`;
    ...
    return {
      recommendedVehicles: (result.recommendedVehicles || []).filter((rv: any) =>
        vehicles.some(v => v.id === rv.vehicleId)
      ),
    };
  } catch (error) {
 ...
 */
export async function personalizePromotion(
  tenantId: string,
  leadId: string,
  promotionId: string,
  apiKey: string
...
- Interacciones: ${interactions.slice(-3).map((i: any) => i.content).join('\n')}
