 ...
 */
export async function suggestVehiclesForLead(
  tenantId: string,
  leadId: string
): Promise<string[] | null> {
  try {
...
export async function generateAIReport(
  tenantId: string,
  reportType: 'leads' | 'sales' | 'inventory' | 'performance',
  filters?: any
): Promise<{
  summary: string;
  insights: string[];
...
${interactions.map((i: any) => `${i.type}: ${i.content}`).join('\n')}
...
export async function optimizeAppointmentSchedule(
  ...
  try {
    ...
    const bookedTimes = appointmentsSnapshot.docs.map(doc => 
