...
/** Prefijo para no confundir tokens de soporte con JWT de Firebase (`eyJ…`). */
export const SUPPORT_TOKEN_PREFIX = 'sup1.';
...
export function encodeSupportSessionToken(payload: SupportSessionTokenPayload): string {
  return `${SUPPORT_TOKEN_PREFIX}${toBase64Url(JSON.stringify(payload))}`;
}

export function isSupportSessionTokenShape(token: string): boolean {
  if (!token) return false;
  if (token.startsWith(SUPPORT_TOKEN_PREFIX)) return true;
  // Legado: base64 JSON con support:true (sin prefijo)
  return tryParseSupportSessionToken(token) !== null;
}
...
export function tryParseSupportSessionToken(token: string): SupportSessionTokenPayload | null {
  ...
  if (trimmed.startsWith(SUPPORT_TOKEN_PREFIX)) {
    candidates.push(trimmed.slice(SUPPORT_TOKEN_PREFIX.length));
  } else {
    // No intentar parsear JWT de Firebase como soporte
    if (trimmed.startsWith('eyJ') && trimmed.split('.').length >= 3) return null;
    candidates.push(trimmed);
  }

  for (const part of candidates) {
    try {
      const decoded = fromBase64Url(part);
