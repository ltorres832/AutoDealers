...
export interface MappedMetaLeadForm {
  name: string;
  email: string;
  phone: string;
  city: string;
  vehicleInterest: string;
  /** Pares con el nombre de campo exacto que envió Meta */
...
function pick(byLower: Map<string, string>, keys: string[]): string {
  for (const k of keys) {
 ...
 */
export function mapMetaLeadFieldDataToLeadForm(
  fieldData: MetaLeadFieldDatum[] | undefined,
  opts?: { leadgenId?: string }
): MappedMetaLeadForm {
  ...
  if (!name) name = 'Lead formulario Meta';
...
export function formatMetaLeadNotes(
  mapped: MappedMetaLeadForm,
  meta?: { formId?: string; adId?: string }
): string {
  const lines = Object.entries(mapped.leadFormResponses).map(([k, v]) => `• ${k}: ${v}`);
  const header = ['Lead recibido desde anuncio (Meta Lead Ads).'];
  if (meta?.formId) header.push(`Form ID: ${meta.formId}`);
  if (meta?.adId) header.push(`Ad ID: ${meta.adId}`);
