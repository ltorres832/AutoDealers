...
export function replaceTemplateVariables(
  ...
  return normalizePlatformMessageText(content);
}
...
export function replaceTemplateSubject(
  ...
  return normalizePlatformMessageText(subject);
}
...
Gracias por haber sido parte de AutoDealersOnline.
...
Equipo AutoDealersOnline`,
    ...
    // Payment Success SMS
    {
      ...
      type: 'sms',
      event: 'payment_success',
      content: 'Hola {{userName}}, tu pago de ${{amount}} para {{membershipName}} fue procesado exitosamente. Gracias! - AutoDealersOnline',
    ...
    // Payment Failed SMS
    {
      ...
      type: 'sms',
      event: 'payment_failed',
      content: 'Hola {{userName}}, no pudimos procesar tu pago de ${{amount}}. Actualiza tu método de pago para evitar suspensión. - AutoDealersOnline',
    ...
    // Payment Reminder 3 Days SMS
    {
      ...
      type: 'sms',
      event: 'payment_reminder_3days',
      content: 'Hola {{userName}}, tu pago de ${{amount}} vence en {{days}} días. Actualiza tu método de pago para evitar suspensión. - AutoDealersOnline',
    ...
    // Payment Reminder 5 Days SMS
    {
      ...
      type: 'sms',
      event: 'payment_reminder_5days',
      content: 'URGENTE {{userName}}: Tu pago de ${{amount}} vence en {{days}} días. Actualiza tu método de pago ahora. - AutoDealersOnline',
      variables: ['userName', 'amount', 'days'],
      isActive: true,
      isDefault: true,
    },
    ...
    // Account Suspended SMS
    {
      ...
      type: 'sms',
      event: 'account_suspended',
      content: 'Hola {{userName}}, tu cuenta fue suspendida por falta de pago. Realiza el pago de ${{amount}} para reactivar. - AutoDealersOnline',
      variables: ['userName', 'amount'],
      isActive: true,
      isDefault: true,
    },
    ...
    // Account Reactivated SMS
    {
      ...
      type: 'sms',
      event: 'account_reactivated',
      content: `Hola {{userName}}, tu cuenta ha sido reactivada exitosamente. Tu pago fue procesado. Gracias! - AutoDealersOnline`,
      variables: ['userName'],
      isActive: true,
      isDefault: true,
    },
...
/** Actualiza plantillas guardadas en Firestore que aún dicen "AutoDealers". */
export async function migrateCommunicationTemplateBranding(): Promise<{
  total: number;
  updated: number;
}> {
  const snap = await getDb().collection('communication_templates').get();
  let updated = 0;
