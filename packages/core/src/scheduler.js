...
exports.classifyNewLeadsWithAI = classifyNewLeadsWithAI;
exports.processActivePromotions = processActivePromotions;
    ...
    for (const tenantDoc of tenantsSnapshot.docs) {
        ...
        for (const reminder of reminders) {
            try {
                ...
                // Marcar como enviado
                await (0, crm_1.markReminderAsSent)(tenantId, reminder.id);
            }
            catch (error) {
...
async function processAppointmentReminders() {
    ...
    for (const tenantDoc of tenantsSnapshot.docs) {
        ...
        for (const aptDoc of appointments24h.docs) {
            ...
            const has24hReminder = apt.reminders?.some((r) => r.sentAt && r.channel && r.type === '24h');
        ...
        for (const aptDoc of appointments1h.docs) {
            ...
            if (!scheduledAt)
                continue;
            const has1hReminder = apt.reminders?.some((r) => r.sentAt && r.channel && r.type === '1h');
 ...
 */
async function classifyNewLeadsWithAI() {
    const { AIClassifier } = await Promise.resolve().then(() => __importStar(require('@autodealers/ai')));
    const { getLeads, updateLead } = await Promise.resolve().then(() => __importStar(require('@autodealers/crm')));
    const tenantsSnapshot = await getDb().collection('tenants').get();
    for (const tenantDoc of tenantsSnapshot.docs) {
        const tenantId = tenantDoc.id;
        // Obtener leads nuevos sin clasificación
        const leads = await getLeads(tenantId, { status: 'new' });
        const unclassifiedLeads = leads.filter((lead) => !lead.aiClassification);
 ...
 */
async function processActivePromotions() {
    const { getActivePromotions } = await Promise.resolve().then(() => __importStar(require('./promotions')));
    ...
    const { getDefaultTemplate, processTemplate } = await Promise.resolve().then(() => __importStar(require('./templates')));
    const tenantsSnapshot = await getDb().collection('tenants').get();
    for (const tenantDoc of tenantsSnapshot.docs) {
        const tenantId = tenantDoc.id;
        try {
            ...
            for (const promotion of activePromotions) {
                ...
                if (promotion.autoSendToLeads) {
                    try {
                        ...
                        const leadsToSend = leads.filter((lead) => lead.status !== 'closed' && lead.status !== 'lost');
                        // Obtener template de promoción del tenant
                        const { getTemplates } = await Promise.resolve().then(() => __importStar(require('./templates')));
                        let templates = await getTemplates(undefined, 'seller');
                        let template = templates.find((t) => (t.type === 'whatsapp' || t.type === 'message') && t.category === 'promotion') || templates.find((t) => t.type === 'whatsapp');
                        if (!template) {
                ...
                if (promotion.autoSendToCustomers) {
                    try {
                        const { getTenantSales } = await Promise.resolve().then(() => __importStar(require('@autodealers/crm')));
...
async function runScheduledTasks() {
    ...
    try {
        ...
        const { runFollowUpCampaigns } = await Promise.resolve().then(() => __importStar(require('./follow-up')));
