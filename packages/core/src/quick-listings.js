...
exports.countActiveQuickListingsByPhone = countActiveQuickListingsByPhone;
exports.createQuickListing = createQuickListing;
exports.listQuickListings = listQuickListings;
exports.getQuickListingById = getQuickListingById;
exports.deleteQuickListing = deleteQuickListing;
exports.purgeExpiredQuickListings = purgeExpiredQuickListings;
exports.incrementQuickListingView = incrementQuickListingView;
const shared_1 = require("@autodealers/shared");
const admin = __importStar(require("firebase-admin"));
const free_public_listings_1 = require("./free-public-listings");
/**
...
function mapDoc(doc) {
    ...
    const photos = Array.isArray(d.photos)
        ? d.photos.filter((p) => typeof p === 'string').slice(0, 12)
        : [];
    return {
        id: doc.id,
        ...
        photos: photos,
        status: (d.status === 'expired' || d.status === 'removed' ? d.status : 'active'),
...
async function countActiveQuickListingsByPhone(phone) {
 ...
 */
async function createQuickListing(input) {
    const settings = await (0, free_public_listings_1.getFreePublicListingsSettings)();
    if (!settings.enabled) {
        return {
            ok: false,
            status: 403,
    ...
    if (!name)
        return { ok: false, status: 400, message: 'Nombre requerido.' };
    if (!phone)
        return { ok: false, status: 400, message: 'Teléfono requerido.' };
    if (!make)
        return { ok: false, status: 400, message: 'Marca requerida.' };
    if (!model)
        return { ok: false, status: 400, message: 'Modelo requerido.' };
    ...
    const photos = Array.isArray(input.photos)
        ? input.photos.filter((p) => typeof p === 'string' && p.startsWith('http')).slice(0, 8)
        : [];
    const now = new Date();
 ...
 */
async function listQuickListings(opts = {}) {
    ...
    for (const d of snap.docs) {
        ...
        if (!m)
            continue;
        if (!opts.includeAll && m.status !== 'active')
            continue;
        if (!opts.includeAll && isExpired(m))
            continue;
        if (opts.city && m.city && opts.city.toLowerCase() !== m.city.toLowerCase())
            continue;
        items.push(m);
...
async function getQuickListingById(id) {
...
async function deleteQuickListing(id) {
 ...
 */
async function purgeExpiredQuickListings() {
...
async function incrementQuickListingView(id) {
    try {
        await getDb()
            .collection(COLLECTION)
            .doc(id)
