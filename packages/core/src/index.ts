// Core Module - Base del sistema
// Autenticación, autorización, multi-tenancy, usuarios

// EXPORTS - FIREBASE PRIMERO (MUY IMPORTANTE)
// Estas funciones antes se re-exportaban aquí. Ahora se deben importar de @autodealers/shared/firebase-server
// o del archivo local lib/firebase-admin.ts

export * from './firestore-utils';
export * from './firebase';

// Resto de exports
export * from './auth';
export * from './users';
export * from './platform-registration';
export * from './user-auth-sync';
export * from './tenants';
export * from './tenant-subdomain';
export { getTenantByWhatsAppNumber } from './tenants';
export * from './ai-config';
export * from './whatsapp-config';
export * from './seller-management';
export * from './roles';
export * from './dms-modules';
// Evitar colisión de nombres: exportar permisos de usuarios con alias
export { hasPermission as hasUserPermission } from './permissions';
export * from './membership-validation';
export * from './memberships-catalog';
export * from './storage';
export * from './sub-users';
export * from './dealer-seller-links';
export * from './dealer-seller-invite-codes';
export * from './seller-limit-allocation';
export * from './newsletter-subscribers';
export * from './auto-responses';
export * from './campaigns';
export * from './faqs';
export * from './social-integrations';
export * from './social-oauth-state';
export * from './meta-oauth-scopes';
export * from './meta-token-health';
export * from './tiktok-oauth';
export * from './youtube-oauth';
export * from './social-video-platforms';
export * from './platform-social';
export * from './platform-meta-system-user';
export * from './registration-facebook-announce';
export * from './registration-social-trigger';
export * from './social-post-image';
export * from './normalize-social-post';
export * from './tenant-ai-automation';
export * from './meta-leadgen-map';
export * from './webhook-config';
export * from './social-ai';
export * from './social-ads';
export * from './meta-vehicle-catalog';
export * from './social-scheduler';
export * from './vehicle-slideshow-video';
export * from './promotions';
export * from './scheduler';
export * from './scheduler-service';
export * from './ai-config';
export * from './follow-up';
// Evitar colisión con admin-users-management: export legacy con alias si se requiere
export {
  createAdminUser as createLegacyAdminUser,
  updateAdminUser as updateLegacyAdminUser,
  deleteAdminUser as deleteLegacyAdminUser,
  getAdminUserById,
  updateAdminUserStatus,
  updateAdminUserPermissions,
} from './admin-users';
export { getAdminUsers } from './admin-users';
export * from './dealer-admin-users';
export * from './feature-executor';
export * from './membership-ai-gate';
export * from './feature-sync';
export * from './feature-middleware';
export * from './dynamic-features';
export * from './feature-executor-enhanced';
export * from './communication-templates';
export * from './communication-sender';
export * from './messaging-outbound';
export * from './email-delivery';
export * from './platform-admin-notify';
export * from './welcome-email';
export * from './platform-support';
export * from './support-sessions';
export * from './staff-temp-access';
export * from './whatsapp-messaging';
export * from './password-reset-email';
export * from './app-passwords';
export * from './communication-logs';
export * from './credentials';
export * from './stripe-helper';
export * from './admin-permissions';
export * from './admin-users-management';
export * from './admin-platform-delete';
export * from './pricing-config';
export * from './featured-promotions';
export * from './free-public-listings';
export * from './crm-pipeline-settings';
export * from './quick-listings';
export * from './contact-inquiries';
export * from './platform-activity';
export * from './exclusive-offers-section';
export * from './inventory-finder-cta';
export * from './why-choose-us-section';
export * from './ratings';
export * from './notifications';
export * from './vin';
export * from './fi-notifications';
export * from './fcm-tokens';
export * from './notification-prefs';
export * from './advertisers';
export * from './advertiser-pricing';
export * from './advertiser-limits';
export * from './advertiser-targeting';
export * from './advertiser-ab-testing';
export * from './advertiser-metrics';
export * from './advertiser-specs';
export * from './ad-placement-dimensions';
export * from './ad-placements';
export * from './ad-creative';
export * from './sponsored-content-visibility';
export * from './sponsored-content-schedule';
export * from './paid-inventory-stripe';
export * from './referrals';
export * from './affiliates';
export * from './stripe-connect-affiliates';
export * from './sales-employees';
export * from './sales-employee-ads';
export * from './stripe-connect-sales-employees';
export * from './stripe-connect-tenants';
export * from './public-api-keys';
export * from './public-api-webhooks';
export * from './integration-apps';
export * from './stripe-admin-infrastructure';
export * from './corporate-email';
export * from './email-aliases';
export * from './multi-dealer-access';
export * from './multi-dealer-account';
export * from './types';
export * from './feature-flags';
export type { FeatureConfig, DashboardType, DashboardFeatures } from './feature-flags';
export * from './dashboard-feature-membership';
export * from './maintenance';
export * from './announcements';
export * from './document-branding';
export * from './pdf-generator';
export * from './pdf-layout';
export * from './custom-documents';
export * from './fi-pdf-documents';
export * from './fi-pdf-service';
export * from './fi-cosigner-payload';
export * from './fi-ssn';
export * from './policies';
export * from './policy-notifications';
export * from './automotive-categories';
export * from './automotive-specializations';
export * from './automotive-business';
export * from './automotive-ops';
export * from './automotive-payments';
export * from './automotive-document-email';
export * from './customer-garage';
