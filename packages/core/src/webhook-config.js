...
exports.resolveMetaWebhookVerifyToken = resolveMetaWebhookVerifyToken;
 ...
 */
async function resolveMetaWebhookVerifyToken() {
    const fromEnv = getFacebookWebhookVerifyToken();
    if (fromEnv !== 'default_verify_token') {
        return fromEnv;
    }
    try {
        const meta = await (0, credentials_1.getMetaCredentials)();
    ...
    try {
        const wa = await (0, credentials_1.getWhatsAppCredentials)();
