...
exports.generateReceiptFromInvoice = generateReceiptFromInvoice;
exports.generateReceiptHTML = generateReceiptHTML;
exports.generateReceiptText = generateReceiptText;
 ...
 */
function generateReceiptFromInvoice(invoice, customerName, customerEmail) {
...
ITEMS:
------
${receipt.items.map(item => `${item.description} - Cantidad: ${item.quantity} x $${item.unitPrice.toFixed(2)} = $${item.subtotal.toFixed(2)}`).join('\n')}
