...
exports.buildVehicleStockSnapshot = buildVehicleStockSnapshot;
...
exports.syncInventoryToWeb = syncInventoryToWeb;
 ...
 */
async function generateStockNumber(tenantId) {
 ...
 */
async function createVehicle(tenantId, vehicleData, sellerId // ID del seller que crea el vehículo (se asigna automáticamente si se proporciona)
) {
    const docRef = getDb()
        .collection('tenants')
        .doc(tenantId)
    ...
    let stockNumber = vehicleData.stockNumber || vehicleData.specifications?.stockNumber;
    ...
    let bodyType = vehicleData.bodyType || vehicleData.specifications?.bodyType;
    ...
    console.log('💾 Guardando vehículo en Firestore:', {
        ...
        bodyTypeSource: vehicleData.bodyType ? 'nivel superior' : vehicleData.specifications?.bodyType ? 'specifications' : 'ninguno',
        bodyTypeOriginal: vehicleData.bodyType || vehicleData.specifications?.bodyType || 'ninguno',
    ...
    const dataToSave = {
        ...
        updatedAt: (0, shared_1.getFirestoreFieldValue)().serverTimestamp(),
    };
 ...
 */
function buildVehicleStockSnapshot(vehicle) {
 ...
 */
async function getVehicleByStockNumber(tenantId, stockNumber) {
 ...
 */
async function getVehicles(tenantId, filters) {
    ...
    if (!hasVehicleFilters(filters)) {
        ...
        if (docs.length > MAX_UNFILTERED_VEHICLE_DOCS) {
            console.warn(`   ⚠️ Tenant ${tenantId}: ${docs.length} vehículos; truncando a ${MAX_UNFILTERED_VEHICLE_DOCS} para esta petición`);
        ...
        const prefetchCap = typeof filters?.prefetchCap === 'number' && filters.prefetchCap > 0
    ...
    if (filters?.search) {
        ...
        vehicles = vehicles.filter((v) => (v.make && String(v.make).toLowerCase().includes(searchLower)) ||
 ...
 */
async function updateVehicle(tenantId, vehicleId, updates) {
 ...
 */
async function updateVehicleStatus(tenantId, vehicleId, status) {
    const updateData = {
        status,
        updatedAt: (0, shared_1.getFirestoreFieldValue)().serverTimestamp(),
    };
    if (status === 'sold') {
        updateData.soldAt = (0, shared_1.getFirestoreFieldValue)().serverTimestamp();
    }
    await getDb()
        .collection('tenants')
        .doc(tenantId)
 ...
 */
async function syncInventoryToWeb(tenantId) {
    ...
    await getDb()
        ...
        lastSync: (0, shared_1.getFirestoreFieldValue)().serverTimestamp(),
    });
}
