// Crear anuncios de prueba en todas las ubicaciones
...
if (!projectId || !clientEmail || !privateKey) {
  console.error('❌ No se encontraron las credenciales de Firebase');
  process.exit(1);
}
...
const placements = [
  { placement: 'hero', title: 'Banner Hero - Parte Superior', description: 'Anuncio en la parte superior de la página (hero section)' },
  { placement: 'sidebar', title: 'Banner Sidebar - Barra Lateral', description: 'Anuncio en la barra lateral de la página' },
  { placement: 'sponsors_section', title: 'Banner Patrocinadores', description: 'Anuncio en la sección de patrocinadores' },
  { placement: 'between_content', title: 'Banner Entre Contenido', description: 'Anuncio entre el contenido principal' },
];

async function createAllAds() {
  try {
    ...
    for (const { placement, title, description } of placements) {
      console.log(`Creando anuncio para: ${placement}...`);
      
      const adRef = await db.collection('sponsored_content').add({
        advertiserId: advertiserId,
        advertiserName: advertiserData?.companyName || 'El Chulo',
        campaignName: `Demo ${placement}`,
        type: 'banner',
        placement: placement,
        title: title,
        description: description,
        imageUrl: `https://via.placeholder.com/900x400?text=${encodeURIComponent(title)}`,
        videoUrl: '',
      ...
      createdAds.push({ id: adRef.id, placement, title });
      console.log(`  ✅ Creado: ${adRef.id}`);
    }
    
    console.log('');
    console.log('✅ Todos los anuncios creados exitosamente!');
    console.log('');
    console.log('📋 Anuncios creados:');
    createdAds.forEach(ad => {
      console.log(`  - ${ad.placement}: ${ad.id} (${ad.title})`);
    });
    console.log('');
    console.log('📍 Deberían verse en: http://localhost:3000/');
    console.log('📊 Estado: active');
    console.log('📅 Fechas válidas hasta:', endDate.toDate().toISOString());
    console.log('');
