...
export function RequestSalesOrientation() {
  ...
  useEffect(() => {
    let cancelled = false;
    fetchWithAuth('/api/sales-orientation')
      .then(async (res) => {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setMsg('');
    try {
      const res = await fetchWithAuth('/api/sales-orientation', {
        method: 'POST',
  ...
  return (
    <form onSubmit={submit} className="bg-white rounded-lg shadow p-4 mb-6 space-y-2">
      <h2 className="font-semibold text-gray-900">Pedir cita de configuración</h2>
