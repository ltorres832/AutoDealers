      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-2">
            Crea gerentes y administradores para gestionar tu negocio. Podrán ver operaciones, recibir notificaciones y gestionar solicitudes.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div key={user.id} className="bg-white rounded-lg shadow p-6">
            ...
            <div className="mb-4">
              ...
              <p className="font-medium">
                {user.role === 'manager' ? '👨‍💼 Gerente' : '👔 Administrador'}
              </p>
            </div>
        ...
        {users.length === 0 && (
          <div className="col-span-full bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
            <p className="text-gray-600 mb-4">No hay gerentes o administradores creados</p>
            <button
              onClick={() => setShowCreateModal(true)}
              className="bg-primary-600 text-white px-6 py-3 rounded-lg hover:bg-primary-700"
            >
              Crear primer gerente/administrador
            </button>
          </div>
        )}
      </div>
...
function CreateUserModal({
  ...
  const [formData, setFormData] = useState({
    ...
    permissions: {
      ...
      canManageUsers: false, // Solo para dealer_admin
    },
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Si es dealer_admin, debe tener canManageUsers
    if (formData.role === 'dealer_admin' && !formData.permissions.canManageUsers) {
      alert('Los administradores deben tener el permiso de gestionar usuarios');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        alert('Usuario creado exitosamente. El usuario recibirá una notificación de bienvenida.');
        onSuccess();
      } else {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b">
          <h2 className="text-2xl font-bold">Crear Gerente/Administrador</h2>
          <p className="text-sm text-gray-600 mt-1">
            Crea un gerente o administrador para gestionar tu negocio. Podrán ver operaciones, recibir notificaciones y gestionar solicitudes.
          </p>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Para recibir notificaciones por SMS y WhatsApp
            </p>
          </div>
          ...
          <div>
            ...
            <select
              value={formData.role}
              onChange={(e) => {
                const newRole = e.target.value as 'manager' | 'dealer_admin';
                setFormData({ 
                  ...formData, 
                  role: newRole,
                  // Si es dealer_admin, activar canManageUsers automáticamente
                  permissions: {
                    ...formData.permissions,
                    canManageUsers: newRole === 'dealer_admin' ? true : formData.permissions.canManageUsers,
                  }
                });
              }}
              className="w-full border rounded px-3 py-2"
              required
            >
              <option value="manager">Gerente - Puede gestionar operaciones del negocio</option>
              <option value="dealer_admin">Administrador - Puede gestionar todo, incluyendo usuarios</option>
            </select>
            <p className="text-xs text-gray-500 mt-1">
          ...
          <div>
            ...
            <div className="space-y-3 border rounded p-4 bg-gray-50">
              ...
              <label className="flex items-center space-x-3 cursor-pointer hover:bg-white p-2 rounded">
                <input
                  type="checkbox"
                  checked={formData.permissions.canManageSellers}
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      permissions: { ...formData.permissions, canManageSellers: e.target.checked },
                    })
                  }
                  className="w-4 h-4 text-primary-600 rounded"
                />
                <div>
                  ...
                  <p className="text-xs text-gray-500">Crear, editar y gestionar vendedores</p>
                </div>
              </label>
              ...
              {formData.role === 'dealer_admin' && (
                <label className="flex items-center space-x-3 cursor-pointer hover:bg-white p-2 rounded bg-blue-50">
                  <input
                    ...
                    className="w-4 h-4 text-primary-600 rounded"
                    disabled={formData.role === 'dealer_admin'}
                  />
                  <div>
                    <span className="text-sm font-medium">Gestionar Usuarios</span>
                    <p className="text-xs text-gray-500">Crear y gestionar otros usuarios (requerido para administradores)</p>
                  </div>
                </label>
              )}
            </div>
