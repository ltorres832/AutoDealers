...
export async function POST(request: NextRequest) {
  try {
    ...
    // Agregar vehicleId y vehiclePrice si existen
    if (body.vehicleId) {
      promotionData.vehicleId = body.vehicleId;
    }
    if (body.vehiclePrice) {
      promotionData.vehiclePrice = body.vehiclePrice;
    }

    // Verificar si el usuario quiere usar un crédito de promoción
    ...
    try {
      await createNotification({
        ...
        message: creditUsed 
          ? `Se ha creado la promoción: ${body.name} (usando crédito de referido)`
          : `Se ha creado la promoción: ${body.name}`,
        channels: ['system'],
