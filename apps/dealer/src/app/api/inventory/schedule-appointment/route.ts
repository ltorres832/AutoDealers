...
/** Lead + cita opcional vinculados a un vehículo (desde inventario dealer). */
export async function POST(request: NextRequest) {
  try {
    ...
    if (!vehicleId || !contact?.name || !contact?.phone || !assignedTo) {
    ...
    const intentLabel =
      intent === 'test_drive_request'
        ? 'Solicitud de prueba de manejo (inventario dealer)'
        : 'Cita desde inventario dealer';
    ...
    try {
      ...
      console.warn('addInteraction schedule-appointment dealer skipped:', e);
    }

    return NextResponse.json({ lead, appointment }, { status: 201 });
  } catch (e: unknown) {
    const msg = e instanceof Error ? e.message : 'Error';
    console.error('schedule-appointment', e);
