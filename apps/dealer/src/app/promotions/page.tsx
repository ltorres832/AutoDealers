...
export default function PromotionsPage() {
  ...
  const [selectedPromotion, setSelectedPromotion] = useState<PaidPromotion | null>(null);
  const [dealerRating, setDealerRating] = useState<number>(0);
  const [dealerRatingCount, setDealerRatingCount] = useState<number>(0);
  ...
  useEffect(() => {
    if (realtimePromotions) {
      ...
      setPromotions(dealerPromotions.map(p => ({
        ...p,
  ...
  async function fetchPaidPromotions() {
    try {
      ...
      if (response.ok) {
        ...
        setPaidPromotions(data.options || []);
      }
    } catch (error) {
  ...
  async function payAssignedPromotion(promotionRequestId: string) {
    try {
      const response = await fetch('/api/promotions/assigned/pay', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ promotionRequestId }),
      });

      const data = await response.json();

      if (response.ok && data.clientSecret) {
        const request = assignedPromotions.find(p => p.id === promotionRequestId);
        setPaymentData({
          clientSecret: data.clientSecret,
          paymentIntentId: data.paymentIntentId,
          requestId: promotionRequestId,
          amount: request?.price || 0,
          description: `Promoción asignada: ${request?.name || 'Promoción premium'}`,
        });
  ...
  async function buyPromotion(promotionScope: 'vehicle' | 'dealer' | 'seller', vehicleId?: string, duration: number = 7) {
    try {
      ...
      if (response.ok && data.clientSecret) {
        ...
        const defaultPrices: Record<string, Record<number, number>> = {
          ...
          dealer: { 3: 49.99, 7: 89.99, 15: 149.99, 30: 199.99 },
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold">Promociones Asignadas por el Admin</h2>
                  <p className="text-sm text-gray-600">
                    Realiza el pago para activar estas promociones premium
                  </p>
                </div>
              </div>
            </div>
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded p-3 mb-3">
                    <div className="flex justify-between items-center">
                      ...
                      <div>
                        ...
                        <div className="font-bold">{promotion.duration} días</div>
                      </div>
                ...
                <div key={promotion.id} className="bg-white rounded-lg border-2 border-yellow-300 p-4">
                  ...
                  {promotion.expiresAt && (
                    <div className="text-xs text-gray-500 mb-2">
                      Expira: {new Date(promotion.expiresAt).toLocaleDateString()}
                    </div>
                  )}
                </div>
              ))}
...
function PaymentHistorySection() {
  ...
  const [filter, setFilter] = useState<'all' | 'promotion' | 'banner'>('all');
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`px-3 py-1 rounded text-sm ${
              filter === 'promotion' ? 'bg-primary-600 text-white' : 'bg-gray-200'
            }`}
          >
            Promociones
          </button>
...
// Modal para comprar promociones pagadas
function BuyPromotionModal({
  ...
  onClose: () => void;
  onBuy: (scope: 'vehicle' | 'dealer' | 'seller', vehicleId?: string, duration?: number, paymentMethodId?: string | null) => void;
  paidPromotions: PaidPromotion[];
}) {
  const [selectedScope, setSelectedScope] = useState<'vehicle' | 'dealer' | 'seller'>('vehicle');
  ...
  const dealerPrices: Record<number, number> = {
    3: 49.99,
    7: 89.99,
    15: 149.99,
    30: 199.99,
  ...
  const getPrice = () => {
    if (selectedScope === 'dealer') {
      return dealerPrices[selectedDuration] || 49.99;
    } else if (selectedScope === 'seller') {
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="flex justify-between items-center">
              ...
              <div className="text-right text-sm text-gray-600">
                ...
                <div>Tipo: {selectedScope === 'vehicle' ? 'Vehículo' : selectedScope === 'dealer' ? 'Dealer' : 'Vendedor'}</div>
              </div>
            </div>
          </div>
...
function CreatePromotionModal({
  ...
  const [hasFreePromotionFeature, setHasFreePromotionFeature] = useState(false);
