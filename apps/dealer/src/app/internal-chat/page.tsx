...
export default function InternalChatPage() {
      ...
      if (response.ok) {
        ...
        const processedConversations = (data.conversations || []).map((conv: any) => ({
          ...
          lastMessageTime: conv.lastMessageTime 
            ...
            : (conv.createdAt ? new Date(conv.createdAt) : null),
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        // Obtener sellers
        const sellersRes = await fetch('/api/sellers', { credentials: 'include' });
        if (sellersRes.ok) {
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToInternalConversations(tenantId, userId, (list: Conversation[]) => {
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToInternalMessages(
      tenantId,
      userId,
      selectedUserId,
      (processedMessages: InternalMessage[]) => {
        setMessages(
        ...
        >
          <div className="p-4 border-b">
            ...
            <button
              ...
              className="w-full bg-primary-600 text-white py-2 px-3 rounded-lg hover:bg-primary-700 transition-colors text-sm font-medium flex items-center justify-center gap-2 mb-2"
            >
                        ...
                        <button
                          onClick={async (e) => {
                            ...
                            if (confirm(`¿Estás seguro de que quieres eliminar la conversación con ${conv.userName}?`)) {
                              try {
                                const response = await fetch(`/api/internal-chat/conversations/delete?userId=${conv.userId}`, {
                                  method: 'DELETE',
                                  credentials: 'include',
                                });
                                if (response.ok) {
        ...
        <div className="flex-1 overflow-y-auto p-6 min-h-0">
          {loading ? (
            ...
            <div className="space-y-6">
              {(Object.entries(usersByRole) as [string, any[]][]).map(([role, roleUsers]) => (
                <div key={role}>
