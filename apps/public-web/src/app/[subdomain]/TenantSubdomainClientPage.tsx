...
export default function TenantSubdomainClientPage() {
  const params = useParams();

  // VERIFICACI├ôN INMEDIATA EN EL SERVIDOR Y CLIENTE: Si estamos en la ra├¡z sin subdominio, NO renderizar esta p├ígina
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      // Si es dominio ra├¡z Y estamos en "/", esta p├ígina NO deber├¡a cargarse
  ...
  // Obtener subdominio de params o detectarlo desde hostname si no est├í en params
  ...
  // Detectar subdominio desde hostname si no est├í en params (para subdominios din├ímicos)
  useEffect(() => {
    // Si ya estamos redirigiendo, no hacer nada m├ís
    ...
    if (typeof window !== 'undefined' && window.location.pathname === '/') {
      ...
      if (isRootDomain) {
        ...
        // Pero si esta p├ígina se carga de todos modos, forzar redirecci├│n
    ...
    // Primero intentar desde params (pero solo si tiene valor v├ílido)
    const rawSub = params?.subdomain;
    const paramSubdomain = Array.isArray(rawSub) ? rawSub[0] : rawSub;
    // Ignorar subdominios t├⌐cnicos de Firebase App Hosting (ej: t-1593654656---public-web-app-obn2bwb27a-uc)
    ...
    if (paramSubdomain && typeof paramSubdomain === 'string' && paramSubdomain.trim() !== '' && paramSubdomain !== 'undefined' && !isTechnicalSubdomain) {
      ...
      // Si no est├í en params, detectar desde hostname (cliente)
      if (typeof window !== 'undefined') {
        const hostname = window.location.hostname;
        const parts = hostname.split('.');
    ...
    if (detectedSubdomain) {
      ...
      // Subdominio t├⌐cnico de App Hosting: redirigir a la p├ígina principal
      window.location.replace('/');
    }
  }, [params]);
  ...
  async function fetchTenantData() {
    ...
    try {
      setLoading(true);
      console.log(`≡ƒîÉ Fetching tenant data for subdomain: "${subdomainStr}"`);
      if (typeof window !== 'undefined') {
        console.log(`≡ƒôì Current URL: ${window.location.href}`);
      }
      console.log(`≡ƒöù API endpoint: /api/tenant/${subdomainStr}`);
      ...
      console.log('≡ƒôª Tenant data received:', {
      ...
      console.log(`Γ£à Estableciendo ${vehiclesList.length} veh├¡culos en el estado`);
      console.log(`≡ƒôï Primeros 3 veh├¡culos a establecer:`, vehiclesList.slice(0, 3).map((v: any) => ({
        id: v.id,
        make: v.make,
        model: v.model,
        year: v.year,
      ...
      if (vehiclesList.length === 0) {
        console.warn('ΓÜá∩╕Å NO SE RECIBIERON VEH├ìCULOS DEL API');
        console.log('≡ƒôª Respuesta completa del API:', {
          hasTenant: !!data.tenant,
          tenantId: data.tenant?.id,
  ...
  if (loading) {
    // Solo mostrar loading si tenemos subdominio v├ílido O si todav├¡a estamos detectando
      ...
      <div className="flex justify-center items-center min-h-screen bg-gray-50 p-4">
        <div className="text-center max-w-md w-full bg-white rounded-lg shadow-lg p-8">
          ...
          <p className="text-gray-600 mb-6">
            {error || 'El subdominio no existe o est├í inactivo'}
          </p>
          ...
          {subdomain && subdomain === 'demo' && (
            <div className="mt-4">
              <p className="text-sm text-gray-600 mb-4">
                El tenant demo deber├¡a crearse autom├íticamente. Si el problema persiste:
              </p>
              <ul className="text-left text-sm text-gray-600 space-y-2 mb-4">
                <li>• Verifica tu conexión a internet</li>
                <li>• Intenta recargar la página</li>
                <li>• Contacta al soporte si el problema continúa</li>
              </ul>
              <button
                onClick={() => {
                  setError(null);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {vehicles.length > 0 && (
        <VehicleCategories
          vehicleCounts={(() => {
            ...
            // IDs v├ílidos de categor├¡as (de VEHICLE_TYPES)
            ...
            vehicles.forEach((v: any) => {
              ...
              if (bodyType) {
                ...
                // Solo contar si es un ID v├ílido
                ...
                // Debug: primeros 5 veh├¡culos con bodyType v├ílido
            ...
            console.log('≡ƒôè Conteos de tipos de veh├¡culos en frontend:', counts);
            console.log('≡ƒöì Debug info:', debugInfo);
            console.log(`ΓÜá∩╕Å Total veh├¡culos: ${vehicles.length}, Con bodyType v├ílido: ${Object.values(counts).reduce((a, b) => a + b, 0)}, Sin bodyType o no reconocido: ${vehicles.length - Object.values(counts).reduce((a, b) => a + b, 0)}`);
            console.log('≡ƒôï IDs de categor├¡as v├ílidos:', validCategoryIds);
      ...
      <PublicPromoVideoGrid
        ...
        titlePrefix={`Video ΓÇö ${tenant.name}`}
      ...
      {tenant.websiteSettings?.sections?.contact?.enabled !== false && (
        <section id="contact" className="bg-gray-100 py-16">
          <div className="container mx-auto px-4">
            <h2 className="text-3xl font-bold mb-8 text-center">
              {tenant.websiteSettings?.sections?.contact?.title || 'Cont├íctanos'}
            </h2>
              ...
              <div className="mt-8 bg-white rounded-lg shadow p-6">
                ...
                <div className="h-64 bg-gray-200 rounded flex items-center justify-center">
                  <p className="text-gray-600">
                    Mapa de Google Maps se mostrar├í aqu├¡
                    <br />
      ...
      {/* Reviews Section - DESPU├ëS de Contact, ANTES de Footer y ChatWidget */}
...
function ContactFormModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (response.ok) {
        alert('┬íGracias por contactarnos! Te responderemos pronto.');
        onClose();
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-md w-full">
        <div className="p-6 border-b flex justify-between items-center">
          <h2 className="text-2xl font-bold">Cont├íctanos</h2>
          <button
            onClick={onClose}
...
// Componente de Rese├▒as para mostrar en la p├ígina p├║blica
        ...
        <div className="text-center mb-12">
          ...
          {reviews.length > 0 ? (
            <>
              ...
              >
                Ver todas las rese├▒as ΓåÆ
              </Link>
            </>
          ) : (
            <p className="text-gray-600">A├║n no hay rese├▒as disponibles. Las rese├▒as de nuestros clientes aparecer├ín aqu├¡ cuando est├⌐n disponibles.</p>
          )}
        </div>
