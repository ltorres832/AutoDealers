...
// Gestor completo de archivos del cliente - Vista unificada de todos los documentos

import { useState, useEffect } from 'react';
import { useRealtimeCustomerFile } from '@/hooks/useRealtimeCustomerFile';
...
export default function CustomerFileManager({
  ...
  const { customerFile: realtimeFile, loading: fileLoading } = useRealtimeCustomerFile(customerId, saleId);

  useEffect(() => {
    if (realtimeFile) {
      setCustomerFile(realtimeFile);
      setLoading(false);
    } else if (!fileLoading && !customerFile) {
      // Si no existe, intentar obtenerlo primero
      fetchCustomerFile();
    }
  }, [realtimeFile, fileLoading, customerId, saleId]);
