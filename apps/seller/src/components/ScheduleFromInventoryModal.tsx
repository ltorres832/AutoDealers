    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto">
        <h2 className="text-xl font-bold mb-2">
          {isTestDrive ? 'Solicitar prueba de manejo' : 'Crear cita (tu inventario)'}
        </h2>
              ...
              <p className="mt-1 text-emerald-800">
                {isTestDrive
                  ? success.hasAppointment
                    ? 'Solicitud de prueba con cita en calendario. Recibirás notificaciones según tu configuración.'
                    : 'Lead de solicitud de prueba sin cita en calendario; queda en tu CRM para dar seguimiento.'
                  : 'Lead asignado a ti con cita en el calendario. Revisa notificaciones y el CRM.'}
              </p>
            </div>
            {success.leadId ? (
              <Link
              ...
              <p className="text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded px-3 py-2">
                No se recibió el ID del lead en la respuesta; revisa Mis leads o citas.
              </p>
            )}
            {success.hasAppointment ? (
              <Link
            ...
            <div>
              ...
              {isTestDrive ? (
                <p className="text-xs text-gray-500 mt-1">
                  Vacío = solo lead de solicitud; con fecha = también cita de prueba en tu calendario.
                </p>
              ) : null}
            </div>
            {!isTestDrive ? (
