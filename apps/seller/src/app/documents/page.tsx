...
export default function DocumentsPage() {
  ...
  const [createMode, setCreateMode] = useState<'blocks' | 'pdf' | 'photo' | null>(null);
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      {loading ? (
        ...
        <>
          ...
          {tab === 'create' && !createMode && (
            <div className="grid gap-4 md:grid-cols-3">
              {[
                ...
                {
                  id: 'pdf' as const,
                  title: 'Subir PDF legal',
                  desc: 'Sube tu formulario oficial y coloca campos rellenables encima.',
                },
                {
                  id: 'photo' as const,
                  title: 'Desde foto (IA)',
          ...
          {tab === 'create' && createMode === 'pdf' && (
            <div className="bg-white rounded-xl border border-gray-200 p-6 max-w-xl space-y-4">
              <h2 className="text-lg font-semibold">Subir PDF legal</h2>
              <p className="text-sm text-gray-600">
                Sube el PDF. Luego podrás mapear campos (VIN, nombre, precio) en una actualización del mapper visual.
                Por ahora se guarda como plantilla PDF lista para sellar campos vía API.
              </p>
              <input
                type="file"
                ...
                onChange={async (e) => {
                  ...
                  try {
                    ...
                    if (!up.ok) throw new Error(upData.error || 'Error subiendo PDF');
                    const res = await fetchWithAuth('/api/documents/templates', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({
                        ...
                        templateDocumentUrl: upData.url,
                        fillableFields: [
                          {
                            id: 'buyer_name',
                            name: 'buyer_name',
                            ...
                            required: true,
                            binding: 'buyer.name',
                            position: { x: 0.15, y: 0.25, width: 0.4, height: 0.03, page: 0 },
                          },
                          {
                            id: 'vin',
                            name: 'vin',
                            ...
                            required: false,
                            binding: 'vehicle.vin',
                            position: { x: 0.15, y: 0.35, width: 0.4, height: 0.03, page: 0 },
                          },
                        ],
                      }),
                    });
                    const data = await res.json();
                    if (!res.ok) throw new Error(data.error || 'Error');
                    setMessage('Plantilla PDF creada');
                    setCreateMode(null);
                    setTab('library');
                    await load();
                  } catch (err) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-xl max-w-lg w-full p-6 shadow-2xl">
            ...
            <ul className="mt-4 text-sm text-gray-700 space-y-2 list-disc pl-5">
              ...
              <li>
                Para máxima fidelidad legal, si tienes el archivo, usa la opción <strong>Subir PDF legal</strong>.
              </li>
