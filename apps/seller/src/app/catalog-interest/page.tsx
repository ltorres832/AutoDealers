...
function normalizeSignal(raw: unknown, index: number): CatalogInterestSignal {
  const r = raw && typeof raw === 'object' ? (raw as Record<string, unknown>) : {};
  let vehicleSummary: VehicleSummary | undefined;
  if (r.vehicleSummary && typeof r.vehicleSummary === 'object') {
    const vs = r.vehicleSummary as Record<string, unknown>;
    vehicleSummary = {
      label: strField(vs.label) || '—',
      stockNumber: strField(vs.stockNumber),
      ...(typeof vs.year === 'number' ? { year: vs.year } : {}),
      ...(strField(vs.make) ? { make: strField(vs.make)! } : {}),
      ...(strField(vs.model) ? { model: strField(vs.model)! } : {}),
    };
  }
  return {
    id: strField(r.id) || `signal-${index}`,
    vehicleId: strField(r.vehicleId),
    vehicleSummary,
    surface: strField(r.surface),
    path: strField(r.path),
    referrer: strField(r.referrer),
    utmSource: strField(r.utmSource),
    utmMedium: strField(r.utmMedium),
    utmCampaign: strField(r.utmCampaign),
    userAgent: strField(r.userAgent),
    ipHash: strField(r.ipHash),
    anonymous: r.anonymous === true,
    hasExplicitContact: r.hasExplicitContact === true,
    createdAt: strField(r.createdAt),
  };
}
...
function CatalogInterestPageInner() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const urlVehicleId = (searchParams.get('vehicleId') || '').trim();
  ...
  const fetchPage = useCallback(
    async (cursor: string | null, append: boolean) => {
      ...
      try {
        ...
        const rawList = Array.isArray(data.signals) ? data.signals : [];
        const batch = rawList.map((row, index) => normalizeSignal(row, index));
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-2 max-w-3xl text-sm leading-relaxed">
            ...
            datos de contacto siguen en{' '}
            <Link href="/leads" className="text-primary-600 hover:underline">
              Leads
            </Link>
            . Paginación en bloques de {PAGE_SIZE}; el CSV exporta todo lo acumulado tras «Cargar más».
          </p>
        </div>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {signals.map((row, index) => (
                  <tr key={`${row.id}-${index}`} className="hover:bg-gray-50/80 align-top">
