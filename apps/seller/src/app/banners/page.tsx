...
export default function BannersPage() {
  ...
  const [paymentType, setPaymentType] = useState<'banner' | null>(null);
  ...
  useEffect(() => {
    if (realtimeBanners) {
      setBanners(realtimeBanners.map(b => ({
        id: b.id,
        imageUrl: b.imageUrl || '',
        title: b.title || '',
        description: b.description || '',
        ctaText: b.ctaText || '',
        linkType: b.linkType || 'vehicle',
        ...
        views: b.views || 0,
        clicks: b.clicks || 0,
        expiresAt: b.expiresAt instanceof Date ? b.expiresAt.toISOString() : (b.expiresAt as any)?.toDate?.()?.toISOString() || b.expiresAt,
        createdAt: b.createdAt instanceof Date ? b.createdAt.toISOString() : (b.createdAt as any)?.toDate?.()?.toISOString() || b.createdAt,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-2">
            {dealerManaged
              ? 'Consulta con tu concesionario los banners activos en la web pública.'
      ...
      {/* Banners Asignados */}
      {!dealerManaged && assignedBanners.length > 0 && (
        <div className="mb-8">
            ...
            {banners.map((banner) => (
              ...
              >
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="text-xs text-gray-500">
                    {banner.views || 0} vistas · {banner.clicks || 0} clics
                  </div>
                </div>
                {banner.expiresAt && (
                  <div className="text-xs text-gray-500">
          ...
          <div className="bg-white rounded-xl shadow-xl p-8 border-2 border-primary-200 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <StripePaymentForm
              ...
              onSuccess={async (paymentIntentId: string) => {
                try {
                  const response = await fetch('/api/banners/confirm-payment', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                      paymentIntentId,
...
function BuyBannerModal({
  ...
  const [formData, setFormData] = useState({
    ...
    description: '',
    ctaText: 'Ver Más',
    linkType: 'seller' as 'vehicle' | 'dealer' | 'seller' | 'filter',
    linkValue: '',
    duration: 7,
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full border rounded px-3 py-2"
            >
              <option value="seller">Página del Vendedor</option>
