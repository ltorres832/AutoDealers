// Enviar email externo desde F&I (vendedor) con PDF profesional adjunto
...
function buildProfessionalEmailHtml(opts: {
  ...
  const paragraphs = opts.body
    ...
    .map(
      (p) =>
        `<p style="margin:0 0 12px;line-height:1.55;color:#1f2937;font-size:15px;">${p.replace(/</g, '&lt;')}</p>`
    )
    .join('');
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08);">
        <tr><td style="background:#0A0A0A;padding:20px 28px;">
          <p style="margin:0;color:#fff;font-size:18px;font-weight:bold;">${opts.senderName}</p>
  ...
  try {
    ...
    if (user.role !== 'seller') {
      return NextResponse.json(
        { error: 'Solo vendedores pueden enviar emails externos desde F&I' },
        { status: 403 }
      );
    }
    ...
    const userName = userDoc.data()?.name || user.email || 'Vendedor';
    ...
    await addFIRequestNoteDirect(
      user.tenantId!,
      id,
      `Email externo enviado por vendedor a ${to}: ${subject}${attachmentNote}`,
      user.userId,
      true
    );
    ...
    console.error('Error en POST /api/fi/requests/[id]/send-external-email (seller):', error);
    return NextResponse.json(
