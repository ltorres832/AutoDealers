...
export async function POST(request: NextRequest) {
  try {
    ...
    const requestRef = db.collection('tenants').doc(auth.tenantId).collection('featured_promotion_requests').doc(requestId);
    ...
    await db.collection('featured_promotions').doc(activeId).set(
      {
        ...
        sellerId: data.sellerId || auth.userId,
        kind: data.kind,
        planId: data.planId,
    ...
    console.error('featured confirm seller:', error);
