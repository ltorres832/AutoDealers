...
export default function WorkflowsList({ tenantId }: WorkflowsListProps) {
  ...
  const { workflows: realtimeWorkflows, loading: workflowsLoading } = useRealtimeWorkflows({
    tenantId,
  });
  ...
  const workflows = tenantId ? realtimeWorkflows : allWorkflows;
  const loading = tenantId ? workflowsLoading : fetchLoading;
    ...
    try {
      ...
      if (!tenantId) {
        fetch('/api/admin/workflows', {
          ...
          .catch(err => console.error('Error reloading workflows:', err));
      }
    } catch (error) {
          ...
          <div className="mb-4">
            ...
            {showAddAction && (
              <div className="mt-4 p-4 border rounded bg-gray-50">
                ...
                {(newAction.type as any) === 'change_status' && (
                  <div className="mb-3">
                ...
                {(newAction.type as any) === 'create_task' && (
                  <>
                    <div className="mb-3">
